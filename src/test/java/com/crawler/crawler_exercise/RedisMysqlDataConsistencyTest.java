...
//        redisTemplate.opsForZSet().incrementScore(LIKE_RANK_KEY, String.valueOf(article3), 1);
    ...
    void bitMapGetSignInfo(){
        ...
        String key2 = buildSignKey(userId, YearMonth.now());
        ...
        List<Long> result = redisTemplate.execute(
                ...
                (RedisCallback<List<Long>>) connection -> {
                    ...
                    byte[] rawKey = key2.getBytes(StandardCharsets.UTF_8);
        ...
        int daysInMonth = YearMonth.now().lengthOfMonth();  // 本月天数，比如 30/31/28
        List<Integer> signList = new ArrayList<>(Collections.nCopies(daysInMonth, 0));
        long bitmap = result.get(0);
        // 从低位到高位，依次判断每一天是否打卡
        for (int i = 0; i < daysInMonth; i++) {
            // 取第 i 位（i = day - 1）
            int bit2 = (int) ((bitmap >> i) & 1);
            signList.set(i, bit2); // 0 或 1
        }
    ...
    private String buildSignKey(Long userId, YearMonth yearMonth) {
        return "user:sign:" + userId + ":" + yearMonth.getYear() + yearMonth.getMonthValue();
    }
